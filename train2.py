...
def train(opt, device):
    save_dir, epochs, batch_size, workers = \
        Path(opt.save_dir), opt.epoch, opt.batch_size, opt.workers
    ...
    num_classes = len(normal_train_dataset.class_ids) - 1
    
    model = Learner(input_dim=2048, drop_p=0.0, mode=opt.mode, num_classes=num_classes).to(device)
    ...
    for epoch in range(last_epoch, opt.epoch):
        ...
        for batch_idx, ((normal_inputs, normal_ids), (anomaly_inputs, anomaly_ids)) in pbar:
            ...
            # input_ids = torch.cat([anomaly_ids, normal_ids], dim=0)
            batch_size = inputs.shape[0]
            inputs = inputs.view(-1, inputs.size(-1)).to(device)
            outputs, fea, logit_ano = model(inputs)
            ...
            if opt.mode == 'amc_ace':
                ...
                logit_ano = logit_ano.view(batch_size, -1, logit_ano.size(-1)).to(device)
                logit_ano1 = logit_ano[:, :32, :]
                logit_ano2 = logit_ano[:, 32:, :]
                logit_ano = torch.cat([logit_ano1, logit_ano2], dim=0)
                ...
                amc_score, mean, abs_prob = get_amc_score2(logit_ano, fea, mean)
                # amc_loss = weaksup_intra_video_loss(amc_score, batch_size, margin=0.5) * opt.alpha
                amc_loss = weaksup_intra_video_loss2(amc_score, abs_prob, anomaly_ids, batch_size, margin=0.5) * opt.alpha
                loss = loss + amc_loss
                    
                
            if RANK in {-1, 0}:
                mloss = (mloss * batch_idx + loss.data.cpu().numpy()) / (batch_idx + 1) 
        ...
        auc = validate.run2(opt, model, normal_test_loader, anomaly_test_loader)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--mode', type=str, help='amc or noamc or amc_ace', default='amc')
    ...
    parser.add_argument('--beta', type=float, help='weighted sum of ace loss', default=0.1) # Abnormal Cross-Entropy
